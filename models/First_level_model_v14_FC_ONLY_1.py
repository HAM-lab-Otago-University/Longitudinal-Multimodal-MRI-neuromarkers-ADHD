...
# Note, random forest was not run in the end.
# Small change in this version to improve run time for wall clock on HP cluster
# n_jobs=1 to avoid memory contention with SLURM array job parallelism on large FC feature space
...
def load_connectivity_data(fold_idx, split, base_dir):
    ...
    original_memory = X.nbytes / 1024 ** 3
    if X.dtype == np.float64:
        X = X.astype(np.float32)
        logger.info(
            f"Converted connectivity features to float32: {original_memory:.2f}GB → {X.nbytes / 1024 ** 3:.2f}GB")

    if y.dtype == np.float64:
        y = y.astype(np.float32)
        logger.info(f"Converted connectivity targets to float32")

    # Make arrays contiguous to prevent XGBoost copying warnings
