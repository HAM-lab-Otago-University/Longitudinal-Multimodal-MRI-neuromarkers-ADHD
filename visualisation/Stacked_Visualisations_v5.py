...
# Output directory
OUTPUT_DIR = '/media/hcs-sci-psy-narun/Jack/First_level_model_aligned/Stacked_model_ridge_v4_EXTREME_conserve_a2000_keeper/Viz_figures_v7_anothercombattempt'

# Phenotype definitions (from the original script)
PHENOTYPE_LABELS = {
    1: "Control",
    2: "Subthreshold",
    3: "ADHD",
...
def create_phenotype_spaghetti_plot(long_df, y_column, title, filename, output_dir):
    """Create spaghetti plot stratified by phenotype with trend lines for Control and ADHD."""
    logger.info(f"Creating phenotype spaghetti plot: {title}")

    plt.figure(figsize=(14, 10))

    # Filter for subjects with phenotype data
    phenotype_df = long_df.dropna(subset=['phenotype'])

    # Plot each phenotype group
    for phenotype in sorted(phenotype_df['phenotype'].unique()):
        ...
        for subject in group_data['ndar_id'].unique():
            ...
            if len(subject_data) > 1:
                # Increased thickness for subject trajectories
                plt.plot(subject_data['age_years'], subject_data[y_column],
                         color=color, alpha=0.3, linewidth=1.5)

            # Removed the label parameter to exclude dots from legend
            plt.scatter(subject_data['age_years'], subject_data[y_column],
    ...
    for phenotype in [1, 3]:  # Control and ADHD only
        ...
        try:
            ...
            plt.plot(age_range, predictions, color=PHENOTYPE_COLORS[phenotype],
                     ...
                     label=f'{PHENOTYPE_LABELS[phenotype]} Trend (R²={model.rsquared:.3f})')
    ...
    plt.legend(by_label.values(), by_label.keys(), fontsize=28, loc='lower right')
...
def create_combined_observed_predicted_spaghetti_plot(long_df, title, filename, output_dir):
    ...
    # Define muted colors for data points and trajectories
    observed_color = '#A5D3F2'  # Muted data color
    predicted_color = 'lightcoral'  # Slightly muted but still visible

    # Define high contrast colors for trend lines
    observed_trend_color = 'blue'  # Bold black for observed trend
    predicted_trend_color = 'darkred'  # Bold dark red for predicted trend
    ...
    # Add trend lines for both observed and predicted with high contrast colors
    for data_type, label in [('true_g', 'Observed G'), ('stacked_pred', 'Predicted G')]:
        if len(long_df) > 10:
            try:
                X = sm.add_constant(long_df['age_years'])
                ...
                # Use high contrast trend line colors
                trend_color = observed_trend_color if data_type == 'true_g' else predicted_trend_color

                plt.plot(age_range, predictions, color=trend_color,
                         linewidth=5, linestyle='--',
                         label=f'{label} Trend (R²={model.rsquared:.3f})')
...
def main():
    ...
    try:
        ...
        # NEW: Create combined observed vs predicted spaghetti plot
        ...
        if phenotype_map:
            ...
            create_phenotype_spaghetti_plot(long_df_with_phenotype, 'true_g',
                                            'Longitudinal Observed G-Factor by Age and Phenotype',
                                            'spaghetti_observed_g_factor_by_phenotype.png', OUTPUT_DIR)

            create_phenotype_spaghetti_plot(long_df_with_phenotype, 'stacked_pred',
                                            'Longitudinal Predicted G-Factor by Age and Phenotype',
                                            'spaghetti_predicted_g_factor_by_phenotype.png', OUTPUT_DIR)
        else:
        ...
        print(f"Overall R²: {stats.pearsonr(stacked_df['true_g'], stacked_df['stacked_pred'])[0] ** 2:.3f}")
