...
This repository contains the analysis code for a longitudinal neuroimaging study using the OREGON ADHD-1000 dataset. The study predicts general cognitive ability (g-factor) from multi-modal brain imaging data (functional connectivity, ALFF, ReHo, and structural MRI) across participants spanning Control, Subthreshold, and ADHD diagnostic groups, using a nested cross-validated stacked machine learning framework.
...
| File | Description |
|---|---|
| `Stacked_Visualisations_v5.py` | Comprehensive figure suite for the stacked model: predicted vs observed scatterplots, density heatmaps, longitudinal spaghetti plots stratified by phenotype |
...
| `first_level_model_visualizations_for_Manu.py` | Predicted vs observed scatterplots and spaghetti plots for all first-level model/modality combinations; for manuscript supplementary figures |
...
| `Feat_importance_network_correlations_g.py` | Variant of the feature importance correlation script with updated output paths; uses HCP MMP parcellation and Cole network organisation |
...
| `cor_plots_within_and_between_v4_no_box.py` | Within-person and between-person correlation plots with confidence intervals |
| `m5_r2_decomposition_plot_corrected.py` | Stacked bar plot of R2mlm decomposition components (fixed within, fixed between, mean variation, residual) |
| `commonality_euler_final.py` | Euler/Venn diagram visualising commonality analysis across within-person trajectories, between-person means, and age components |
...
Scripts target the Aoraki cluster (SLURM) using Apptainer containers for fMRIPrep 24.0.0 and XCP-D 0.10.6.
